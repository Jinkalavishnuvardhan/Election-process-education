...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const adminIdExists = await Admin.findOne({ where: { adminId } });
        if (adminIdExists) {
...
try { 
  ...
  // Create a JWT token
  const payload = { adminId: admin.adminId, email: admin.email, name:admin.name }; // The payload can include the user's ID or other relevant info
